...
// Bureau — PAIR WALL.
// Same body grid as Glyph (left column: headline + dek +
// categories tucked under a hairline; right column: form), but
// without the giant architectural wordmark crowning the page.
// Standard renner mark in the header. The wall reads as a
// quieter version of Glyph — same composition logic, less
// brand-forward.

export function BureauPairBody({ tone }: { tone: ShellTone }) {
  return (
...
function Body() {
  return (
    ...
    >
      <div className="bureau-pair-body" style={{ maxWidth: 1440, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(0, 1fr)", gap: "clamp(40px, 6vw, 96px)", alignItems: "start" }}>
        <div>
          <h1
            style={{
              fontFamily: SERIF,
      ...
      <style>{`
        @media (max-width: 880px) {
          .bureau-pair-body { grid-template-columns: 1fr !important; gap: 40px !important; }
        }
      `}</style>
    </section>
  );
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "clamp(14px, 1.8vw, 20px)" }}>
        <PairField label="First" type="text" placeholder="James" autoComplete="given-name" />
        <PairField label="Last" type="text" placeholder="Mendez" autoComplete="family-name" />
      </div>
      <PairField label="Email" type="email" placeholder="you@firm.com" autoComplete="email" />
      <PairField label="Password" type="password" placeholder="At least 8 characters" autoComplete="new-password" />
