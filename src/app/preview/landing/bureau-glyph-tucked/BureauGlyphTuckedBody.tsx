...
// Bureau — GLYPH WALL · TUCKED.
// Same architectural wordmark as the original Glyph, but the
// category strip is tucked inside the body's headline column,
// after the dek. The reading order goes:
//
//     giant glyph → "Keep real estate running." → dek →
//     categories → form
//
// The headline keeps top billing (no list of categories
// crowding in front of it), and the categories appear as a
// supporting list close to the message they support.

export function BureauGlyphTuckedBody({ tone }: { tone: ShellTone }) {
  return (
      ...
      <div className="bureau-gl-body" style={{ maxWidth: 1440, margin: "0 auto", display: "grid", gridTemplateColumns: "minmax(0, 1.1fr) minmax(0, 1fr)", gap: "clamp(40px, 6vw, 96px)", alignItems: "start" }}>
        <div>
          ...
          {/* Categories tucked right under the dek, supporting
              the headline rather than preceding it. Mono caps
              with middots, separated from headline+dek by a
              single light hairline above. */}
