...
// COMPACT — every wall element packed into a single bordered
// container, dense griddled rows. Top row: headline + dek as
// the lede. Middle row: categories as a thin horizontal band.
// Bottom row: signup heading + form. Reads as a printed table,
// the DNA of how-it-works Compact.

export function BureauCompactBody({ tone }: { tone: ShellTone }) {
  return (
    <div style={{ ...getToneVars(tone), backgroundColor: PAPER, color: INK, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        >
          {/* Lede row */}
          <div style={{ padding: "clamp(40px, 5vw, 64px) clamp(28px, 3.5vw, 56px)" }}>
            <h1
              style={{
                fontFamily: SERIF,
                fontWeight: 400,
          ...
          {/* Categories row */}
          <div style={{ padding: "clamp(20px, 2.4vw, 28px) clamp(28px, 3.5vw, 56px)", borderTop: `1px solid ${INK}`, borderBottom: `1px solid ${INK}` }}>
