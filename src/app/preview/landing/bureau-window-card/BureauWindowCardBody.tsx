...
// Bureau — WINDOW WALL · CARD.
// Same panel-on-chalk composition as Window, but the renner
// mark and Sign in link sit OUTSIDE the panel — at the page
// level, in their normal header position. The panel becomes a
// pure content card holding only the headline, dek, categories,
// and form.
//
// Reads as: standard page chrome, with the wall content laid on
// top as a card. Closer to how the other walls handle the mark
// + sign-in (outside the body), while keeping Window's
// signature panel-on-chalk gesture.

export function BureauWindowCardBody({ tone }: { tone: ShellTone }) {
  return (
    <div style={{ ...getToneVars(tone), backgroundColor: CHALK, color: INK, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <main
        style={{
          ...
          padding: "clamp(24px, 3.5vw, 56px) clamp(20px, 3vw, 48px) clamp(40px, 5vw, 80px)",
        }}
      >
        <Panel />
      </main>
